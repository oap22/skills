...
| | |
|---|---|
| SSH host alias | `ROSIE` → `dh-mgmt2.hpc.msoe.edu` (already in `~/.ssh/config`) |
| User | `pacettio@ad.msoe.edu` |
| Home | `/home/ad.msoe.edu/pacettio` |
...
**`Could not resolve hostname dh-mgmt2.hpc.msoe.edu` means the VPN is off.** It is not a cluster outage. Verified 2026-08-12 — this exact failure occurred before the VPN was connected, and connecting it fixed it.
